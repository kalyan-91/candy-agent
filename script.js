...
(function () {
  ...
  style.textContent = `
    ...
    #csm-sun {
      bottom: 120px;
      right: 24px;
      background: radial-gradient(circle at 35% 35%, #ffe89a, #ff9d00 55%, #ff4f00);
      box-shadow:
        0 0 0 4px rgba(255,180,0,0.2),
        0 0 24px 8px rgba(255,140,0,0.45),
        0 0 60px 20px rgba(255,80,0,0.25),
        0 0 100px 40px rgba(255,120,0,0.12);
      animation: csm-sun-pulse 4s ease-in-out infinite;
    }
    @keyframes csm-sun-pulse {
      0%,100% { box-shadow: 0 0 0 4px rgba(255,180,0,0.2), 0 0 24px 8px rgba(255,140,0,0.45), 0 0 60px 20px rgba(255,80,0,0.25), 0 0 100px 40px rgba(255,120,0,0.12); }
      50%      { box-shadow: 0 0 0 8px rgba(255,180,0,0.3), 0 0 40px 14px rgba(255,140,0,0.6),  0 0 80px 30px rgba(255,80,0,0.32),  0 0 140px 60px rgba(255,120,0,0.18); }
    }
    #csm-sun .csm-icon { font-size: 28px; line-height:1; }

    /* ── Moon ── */
    #csm-moon {
      bottom: 200px;
      right: 24px;
      background: radial-gradient(circle at 40% 30%, #c8d8ff, #7b8fff 50%, #4b2fa0);
      box-shadow:
        0 0 0 4px rgba(130,100,255,0.2),
        0 0 24px 8px rgba(100,80,255,0.4),
        0 0 60px 20px rgba(80,50,220,0.22),
        0 0 100px 40px rgba(60,30,180,0.1);
      animation: csm-moon-pulse 5s ease-in-out infinite;
    }
    @keyframes csm-moon-pulse {
      0%,100% { box-shadow: 0 0 0 4px rgba(130,100,255,0.2), 0 0 24px 8px rgba(100,80,255,0.4), 0 0 60px 20px rgba(80,50,220,0.22), 0 0 100px 40px rgba(60,30,180,0.1); }
      50%      { box-shadow: 0 0 0 8px rgba(130,100,255,0.3), 0 0 40px 14px rgba(100,80,255,0.55), 0 0 80px 30px rgba(80,50,220,0.3), 0 0 140px 60px rgba(60,30,180,0.16); }
    }
    #csm-moon .csm-icon { font-size: 26px; line-height:1; }
    ...
    #csm-sun-panel {
      bottom: 100px;
      background: linear-gradient(145deg, rgba(30,15,5,0.95) 0%, rgba(60,25,0,0.92) 100%);
      box-shadow: 0 8px 40px rgba(255,100,0,0.35), 0 0 0 1px rgba(255,150,50,0.15), inset 0 0 60px rgba(255,80,0,0.05);
    }

    /* ── Moon Panel ── */
    #csm-moon-panel {
      bottom: 180px;
      background: linear-gradient(145deg, rgba(8,5,30,0.96) 0%, rgba(20,10,60,0.93) 100%);
    ...
    #csm-sun-panel .csm-panel-avatar {
      background: radial-gradient(circle at 35% 35%, #ffe89a, #ff9d00);
      box-shadow: 0 0 16px rgba(255,140,0,0.6);
    }
    #csm-moon-panel .csm-panel-avatar {
      background: radial-gradient(circle at 40% 30%, #c8d8ff, #7b8fff);
      box-shadow: 0 0 16px rgba(120,100,255,0.6);
    }
    .csm-panel-titles { flex: 1; }
    .csm-panel-title {
    ...
    #csm-sun-panel .csm-panel-title { color: #ffc060; }
    #csm-moon-panel .csm-panel-title { color: #b0a0ff; }
    .csm-panel-sub {
      font-size: 11px;
    ...
    .csm-energy-label {
      ...
      color: rgba(255,255,255,0.35); white-space: nowrap; font-family: monospace;
    }
    .csm-energy-track {
    ...
    .csm-energy-fill {
      ...
      background: linear-gradient(90deg, #ff6000, #ffc000);
      animation: csm-energy-flow 2s ease-in-out infinite alternate;
    }
    @keyframes csm-energy-flow {
    ...
    .csm-insight {
      ...
      animation: csm-fade-in 0.4s ease both;
    }
    ...
    #csm-sun-panel .csm-action-btn {
      background: rgba(255,120,0,0.12);
      color: #ffaa50;
      border-color: rgba(255,120,0,0.2);
    }
    #csm-sun-panel .csm-action-btn:hover {
      background: rgba(255,120,0,0.25);
      box-shadow: 0 0 20px rgba(255,100,0,0.3);
    }
    #csm-moon-panel .csm-action-btn {
      background: rgba(100,70,255,0.12);
      color: #c0b0ff;
      border-color: rgba(100,70,255,0.2);
    }
    #csm-moon-panel .csm-action-btn:hover {
      background: rgba(100,70,255,0.25);
    ...
    .csm-dream-stream::before {
      content: '';
      position: absolute; inset: 0;
      background: linear-gradient(135deg, transparent 60%, rgba(100,60,255,0.05));
      pointer-events: none;
    }
    .csm-dream-text {
    ...
    .csm-dream-cursor {
      ...
      background: #9080ff; vertical-align: middle;
      margin-left: 2px;
      animation: csm-blink 1s step-end infinite;
    }
    ...
    .csm-badge {
      ...
      border: 2px solid rgba(10,5,30,0.9);
    }
    #csm-sun .csm-badge { background: #ff4400; color: #fff; }
    #csm-moon .csm-badge { background: #7040ff; color: #fff; }

    /* ── Mobile ── */
    @media (max-width: 600px) {
      #csm-sun  { bottom: 80px;  right: 16px; width: 54px; height: 54px; }
      #csm-moon { bottom: 146px; right: 16px; width: 54px; height: 54px; }
      .csm-panel { right: 10px; width: calc(100vw - 20px); }
      #csm-sun-panel  { bottom: 72px; }
      #csm-moon-panel { bottom: 152px; }
  ...
  const SUN_DATA = {
    energy: 94,
    stats: [
      { num: '6', lbl: 'Live Projects' },
      { num: '∞', lbl: 'Knowledge Nodes' },
      { num: '3.3B', lbl: 'Model Params' },
      { num: '99%', lbl: 'Uptime' }
    ],
    insights: [
      { icon: '⚡', tag: 'AI Core', text: 'LLaMA 3.3-70B via Groq API — blazing fast inference at 300+ tokens/sec' },
      { icon: '🔥', tag: 'Solar Flare', text: 'Candy AI processing 847 user sessions today across all features' },
      { icon: '🌐', tag: 'Network', text: 'Cloudflare KV analytics tracking live visitors across 12 regions' },
      { icon: '💡', tag: 'Insight', text: 'Dream Destination Map — most interacted feature this week' }
    ]
  };

  const MOON_DATA = {
    dreams: [
      '"What if Candy could paint your emotions as a nebula in real time..."',
      '"A voice-only mode — speak to the cosmos, receive cosmic wisdom..."',
      '"Project DNA sequencer: visualise each project as a living organism..."',
      '"Time-capsule messages: leave a note for future-you, delivered by starlight..."',
      '"Multiplayer space: two users, one galaxy, shared AI thoughts..."'
    ],
    insights: [
      { icon: '🌙', tag: 'Dream Portal', text: 'Next vision: AR star overlay so users can tag real stars with AI memories' },
      { icon: '🔭', tag: 'Experimental', text: 'Telugu language micro-model — cultural NLP as a portfolio differentiator' },
      { icon: '🧬', tag: 'Future', text: 'BizPilot AI autonomous commerce engine — shipping Q3 2025' },
      { icon: '💫', tag: 'Memory', text: 'LYRA AI mood-reactive soundscapes — syncing Spotify with star colours' }
    ]
  };

  /* ── 3. ROOT MOUNT ─────────────────────────────────────── */
  const root = document.createElement('div');
  root.id = 'candy-sun-moon';
  document.body.appendChild(root);

  /* ── 4. PARTICLE CANVAS ────────────────────────────────── */
  const canvas = document.createElement('canvas');
  canvas.id = 'csm-particle-canvas';
  ...
  function spawnBurst(x, y, color, count = 28) {
  ...
  function spawnMoonBurst(x, y) {
    spawnBurst(x, y, '#9080ff', 35);
    spawnBurst(x, y, '#c0b0ff', 15);
  }
  ...
  sun.id = 'csm-sun';
  sun.className = 'csm-body';
  sun.title = 'Sun — AI Core';
  sun.innerHTML = `
    <span class="csm-icon">☀️</span>
    <span class="csm-badge">★</span>
  `;
  ...
  moon.id = 'csm-moon';
  moon.className = 'csm-body';
  moon.title = 'Moon — Dream Portal';
  moon.innerHTML = `
    <span class="csm-icon">🌙</span>
    <span class="csm-badge">✦</span>
  `;
  ...
  /* ── 8. TOOLTIPS ───────────────────────────────────────── */
  const sunTip = document.createElement('div');
  sunTip.id = 'csm-sun-tooltip';
  sunTip.className = 'csm-tooltip';
  sunTip.textContent = '☀ AI Core & Knowledge Star';
  document.body.appendChild(sunTip);

  const moonTip = document.createElement('div');
  moonTip.id = 'csm-moon-tooltip';
  moonTip.className = 'csm-tooltip';
  moonTip.textContent = '🌙 Dream Portal & Future Visions';
  document.body.appendChild(moonTip);

  sun.addEventListener('mouseenter', () => { sunTip.style.opacity = '1'; });
  sun.addEventListener('mouseleave', () => { sunTip.style.opacity = '0'; });
  moon.addEventListener('mouseenter', () => { moonTip.style.opacity = '1'; });
  moon.addEventListener('mouseleave', () => { moonTip.style.opacity = '0'; });

  /* ── 9. BUILD SUN PANEL ────────────────────────────────── */
  function buildSunPanel() {
    const p = document.createElement('div');
    p.id = 'csm-sun-panel';
    p.className = 'csm-panel';
    p.innerHTML = `
      <div class="csm-panel-header">
        <div class="csm-panel-avatar">☀️</div>
        <div class="csm-panel-titles">
          <p class="csm-panel-title">AI Core — Sol</p>
          <p class="csm-panel-sub">CANDY KNOWLEDGE STAR · ONLINE</p>
        </div>
        <div class="csm-panel-close" id="csm-sun-close">✕</div>
      </div>
      <div class="csm-panel-body">
        <div class="csm-energy-row">
          <span class="csm-energy-label">Energy</span>
          <div class="csm-energy-track">
            <div class="csm-energy-fill" style="width:${SUN_DATA.energy}%"></div>
          </div>
          <span class="csm-energy-val">${SUN_DATA.energy}%</span>
        </div>
        <div class="csm-stats">
          ${SUN_DATA.stats.map(s => `
            <div class="csm-stat-card">
              <div class="csm-stat-num">${s.num}</div>
              <div class="csm-stat-lbl">${s.lbl}</div>
            </div>
          `).join('')}
        </div>
        ${SUN_DATA.insights.map((ins, i) => `
          <div class="csm-insight" style="animation-delay:${i * 0.07}s">
            <span class="csm-insight-icon">${ins.icon}</span>
            <div>
              <span class="csm-insight-tag">${ins.tag}</span>
              <div class="csm-insight-text">${ins.text}</div>
            </div>
          </div>
        `).join('')}
        <button class="csm-action-btn" id="csm-flare-btn">⚡ Trigger Solar Flare</button>
      </div>
    `;
    document.body.appendChild(p);
    document.getElementById('csm-sun-close').onclick = closeSunPanel;
    document.getElementById('csm-flare-btn').onclick = triggerSolarFlare;
    return p;
  }
  ...
  function buildMoonPanel() {
    ...
    p.innerHTML = `
      <div class="csm-panel-header">
        <div class="csm-panel-avatar">🌙</div>
        <div class="csm-panel-titles">
          <p class="csm-panel-title">Dream Portal — Luna</p>
          <p class="csm-panel-sub">FUTURE VISIONS · DREAMING...</p>
        </div>
        <div class="csm-panel-close" id="csm-moon-close">✕</div>
      </div>
      <div class="csm-panel-body">
        <div class="csm-dream-stream">
          <div id="csm-dream-text" class="csm-dream-text">&nbsp;<span class="csm-dream-cursor"></span></div>
        </div>
        ${MOON_DATA.insights.map((ins, i) => `
          <div class="csm-insight" style="animation-delay:${i * 0.07}s">
            <span class="csm-insight-icon">${ins.icon}</span>
            <div>
              <span class="csm-insight-tag">${ins.tag}</span>
        ...
        `).join('')}
        <button class="csm-action-btn" id="csm-dream-next-btn">✦ Channel Next Vision</button>
      </div>
    `;
    document.body.appendChild(p);
    document.getElementById('csm-moon-close').onclick = closeMoonPanel;
    document.getElementById('csm-dream-next-btn').onclick = nextDream;
    return p;
  }
  ...
  function typeWrite(el, text, speed = 28) {
    ...
    let i = 0;
    el.innerHTML = '<span class="csm-dream-cursor"></span>';
    typeTimer = setInterval(() => {
      if (i < text.length) {
        el.innerHTML = text.slice(0, ++i) + '<span class="csm-dream-cursor"></span>';
      } else {
  ...
  function nextDream() {
    const el = document.getElementById('csm-dream-text');
    if (!el) return;
    typeWrite(el, MOON_DATA.dreams[dreamIdx % MOON_DATA.dreams.length]);
    dreamIdx++;
    spawnMoonBurst(
      moon.getBoundingClientRect().left + 27,
      moon.getBoundingClientRect().top  + 27
    );
  }
  ...
  function triggerSolarFlare() {
    ...
    spawnFlare(cx, cy);
    createRipple(sun, 'rgba(255,160,0,0.6)');
    ...
    fs.textContent = '@keyframes csm-flare-fade{to{opacity:0}}';
    document.head.appendChild(fs);
    document.body.appendChild(flash);
    setTimeout(() => { flash.remove(); fs.remove(); }, 800);
  }
  ...
  function openSunPanel() {
    if (!sunPanel) sunPanel = buildSunPanel();
    sunPanel.classList.add('csm-open');
    sunOpen = true;
    const r = sun.getBoundingClientRect();
    spawnBurst(r.left + 32, r.top + 32, '#ffc050', 20);
    createRipple(sun, 'rgba(255,160,0,0.5)');
    sunTip.style.opacity = '0';
  }
  function closeSunPanel() {
    if (!sunPanel) return;
    sunPanel.classList.remove('csm-open');
    sunOpen = false;
  }
  function openMoonPanel() {
    if (!moonPanel) moonPanel = buildMoonPanel();
    moonPanel.classList.add('csm-open');
    moonOpen = true;
    const r = moon.getBoundingClientRect();
    spawnMoonBurst(r.left + 27, r.top + 27);
    createRipple(moon, 'rgba(100,70,255,0.5)');
    moonTip.style.opacity = '0';
    setTimeout(nextDream, 200);
  }
  function closeMoonPanel() {
    if (!moonPanel) return;
    moonPanel.classList.remove('csm-open');
    moonOpen = false;
    if (typeTimer) clearInterval(typeTimer);
  }
  ...
  sun.addEventListener('click', (e) => {
    e.stopPropagation();
    if (sunOpen) closeSunPanel(); else openSunPanel();
    if (moonOpen) closeMoonPanel();
  });
  moon.addEventListener('click', (e) => {
    e.stopPropagation();
    if (moonOpen) closeMoonPanel(); else openMoonPanel();
    if (sunOpen) closeSunPanel();
  });

  document.addEventListener('click', (e) => {
    if (sunOpen  && sunPanel  && !sunPanel.contains(e.target)  && e.target !== sun)  closeSunPanel();
    if (moonOpen && moonPanel && !moonPanel.contains(e.target) && e.target !== moon) closeMoonPanel();
  });

  /* ── 14. AMBIENT STARS ──────────────────────────────────── */
  for (let i = 0; i < 40; i++) {
    const s = document.createElement('div');
    s.className = 'csm-star';
    const size = 1 + Math.random() * 2;
    s.style.cssText = `
      width:${size}px; height:${size}px;
      top:${Math.random() * 100}vh;
      left:${Math.random() * 100}vw;
      --lo: ${0.05 + Math.random() * 0.2};
  ...
  /* ── 15. LIVE ENERGY TICK ──────────────────────────────── */
  setInterval(() => {
    const fill = document.querySelector('#csm-sun-panel .csm-energy-fill');
    const val  = document.querySelector('#csm-sun-panel .csm-energy-val');
    if (!fill || !val) return;
    const nv = Math.max(60, Math.min(100, SUN_DATA.energy + Math.round((Math.random() - 0.5) * 6)));
    fill.style.width = nv + '%';
    val.textContent  = nv + '%';
  }, 3000);

  /* ── 16. MOBILE: swipe to close ────────────────────────── */
  let touchStartX = 0;
