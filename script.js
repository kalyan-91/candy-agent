...
/* ══════════ AI CINEMATIC MODE ══════════ */
const CINEMATIC_PROJECTS = {
  sparms: {
    name: 'SPARMS',
    fullName: 'Student Performance Analysis & Result Management System',
    tech: ['Java Swing', 'MySQL', 'JDBC', 'Maven', 'iText PDF', 'ZXing'],
    problem: 'Manual academic result management was error-prone, slow, and lacked role-based access for admins, faculty, and students.',
    result: 'Fully automated result system with OMR scanning, grade computation, PDF export, and role-based dashboards.',
    color: '#fb7185',
    progress: 92,
  },
  inventoryiq: {
    name: 'InventoryIQ',
    fullName: 'E-Commerce Inventory Analytics System',
    tech: ['Python', 'Streamlit', 'Pandas', 'Plotly'],
    problem: 'Businesses lacked real-time inventory visibility with actionable analytics and secure multi-user access.',
    result: 'Live dashboard with secure login, product management, audit logs, CSV export, and interactive charts.',
    color: '#22d3ee',
    progress: 88,
    link: 'https://inventoryiq-e-commerce-inventory-analytics-system-lqpsn7qy8hhd.streamlit.app',
  },
  digit: {
    name: 'Digit Recognizer',
    fullName: 'Handwritten Digit Recognition CNN App',
    tech: ['Python', 'TensorFlow', 'Streamlit', 'OpenCV'],
    problem: 'Demonstrating deep learning capabilities through a practical, interactive visual application.',
    result: '99%+ accuracy CNN model deployed as a live interactive canvas where users draw digits to test AI.',
    color: '#a78bfa',
    progress: 95,
    link: 'https://hand-written-digit-recognition-xp9dvpheswt6zju8xpknxn.streamlit.app',
  },
  netflix: {
    name: 'Netflix Dashboard',
    fullName: 'Netflix Content Analytics Power BI Dashboard',
    tech: ['Power BI', 'DAX', 'Power Query'],
    problem: 'No visual way to explore 5000+ Netflix titles by genre, duration, country, and release trends.',
    result: 'Interactive Power BI dashboard with slicers, KPIs, and geographic breakdowns of global content.',
    color: '#f43f5e',
    progress: 85,
  },
  attrition: {
    name: 'Employee Attrition',
    fullName: 'Employee Attrition & Engagement Analysis',
    tech: ['Python', 'Scikit-learn', 'Power BI', 'Pandas'],
    problem: 'HR teams struggled to predict employee attrition before it impacted productivity and costs.',
    result: 'ML classification model with 87% accuracy plus Power BI dashboard for HR decision making.',
    color: '#34d399',
    progress: 87,
  },
  zomato: {
    name: 'Zomato Analysis',
    fullName: 'Restaurant Rating & Predictive Analysis',
    tech: ['Python', 'Pandas', 'Scikit-learn', 'Excel'],
    problem: 'Identifying what factors drive restaurant ratings and predicting customer preferences.',
    result: 'EDA revealed top rating patterns with a predictive classification model for restaurant success.',
    color: '#fbbf24',
    progress: 83,
  },
};
...
function detectProjectFromText(text) {
  ...
  if (t.includes('sparms') || t.includes('student performance') || t.includes('result management')) return 'sparms';
  if (t.includes('inventoryiq') || t.includes('inventory')) return 'inventoryiq';
  if (t.includes('digit') || t.includes('handwritten') || t.includes('cnn')) return 'digit';
  if (t.includes('netflix')) return 'netflix';
  if (t.includes('attrition') || t.includes('employee')) return 'attrition';
  if (t.includes('zomato') || t.includes('restaurant')) return 'zomato';
  return null;
}

function launchCinematic(projectKey) {
  const p = CINEMATIC_PROJECTS[projectKey];
  if (!p) return;

  const overlay = document.getElementById('cinematicOverlay');
  const body    = document.getElementById('cinBody');
  const label   = document.getElementById('cinLabel');
  const sublabel = document.getElementById('cinSublabel');
  const closeBtn = document.getElementById('cinClose');
  if (!overlay) return;

  // Reset
  body.innerHTML = '';
  closeBtn.classList.remove('visible');
  label.textContent = 'PROJECT DETECTED';
  sublabel.textContent = 'Initializing cinematic briefing...';

  overlay.classList.add('active');

  // Step 1 — Label animation
  setTimeout(() => {
    label.textContent = p.name.toUpperCase();
    sublabel.textContent = p.fullName;
  }, 600);
  ...
  cards.forEach(card => {
    ...
    el.className = 'cin-card';
    el.style.setProperty('--cin-color', card.color);

    if (card.badges) {
      el.innerHTML = `
        <div class="cin-card-tag">${card.tag}</div>
        <div class="cin-badges">
          ${card.badges.map(b => `<span class="cin-badge">${b}</span>`).join('')}
        </div>`;
    } else {
      el.innerHTML = `
        <div class="cin-card-tag">${card.tag}</div>
        <div class="cin-card-value">${card.value}</div>
        ${card.progress ? `<div class="cin-progress-bar"><div class="cin-progress-fill" style="--target:${card.progress}%"></div></div>` : ''}`;
    }
    ...
    setTimeout(() => {
      ...
      if (card.badges) {
        el.querySelectorAll('.cin-badge').forEach((b, i) => {
          setTimeout(() => b.classList.add('visible'), i * 80);
        });
      }
      if (card.progress) {
        setTimeout(() => {
          const fill = el.querySelector('.cin-progress-fill');
  ...
  setTimeout(() => {
    ...
    if (p.link) {
      ...
      linkEl.className = 'cin-live-link';
      linkEl.innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="13" height="13"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg> VIEW LIVE MISSION`;
      body.appendChild(linkEl);

      // Add live link CSS inline
      linkEl.style.cssText = `display:inline-flex;align-items:center;gap:8px;padding:9px 20px;border-radius:100px;background:rgba(0,212,255,0.08);border:1px solid rgba(0,212,255,0.28);color:#00d4ff;font-family:'JetBrains Mono',monospace;font-size:0.65rem;letter-spacing:0.1em;text-decoration:none;text-transform:uppercase;transition:all 0.22s;margin-top:8px;`;
    }
  }, 3600);
